...
#include <stdio.h>	// plan: use popen("ppm2pcx -packed ...","w"); for writing pcx
#include <stdlib.h>
// #include <assert.h>
...
char read_b(FILE *f1){
  char c=0; c=fgetc(f1); if(feof(f1) || ferror(f1))err=1; return c;
}
...
void writebmp(char *name,pix p,int vvv){ 	// see pcx.format.txt
  ...
  // 54 - endofheader
  for(i=0;i<256;i++){ 
     h[54+4*i+0]=((~((i & 16)*8)) &                    (i & (128+64)))|63;
     h[54+4*i+1]=((~((i & 16)*8)) &  (~((i & 32)*4)) & (i & (128+64)))|63;
     h[54+4*i+2]=(  ((i & 16)*4)  | ((~((i & 32)*4)) & (i & (128+64)))|63);
  }			 // blue-green-red
