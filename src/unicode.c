...
const char *decode(wchar_t c, FORMAT type) {
  ...
  switch (type) {
	...
	default:
	  /* snprintf seems to be no standart, so I use insecure sprintf */
    ...
    default: /* use UTF8 as default, test with xterm -u8 */
      ...
      if ( c <= 0x7F ) { /* UTF8 == 7bit ASCII */
        buf[0] = (char)c;
        return buf;
      }
      if ( c <= (wchar_t)0x7FF ) { /* UTF8 == 11bit */
        buf[0] = (char)(0xc0+(c>>6 & 0x1f)); /* 110xxxxx */
        buf[1] = (char)(0x80+(c    & 0x3f)); /* 10xxxxxx */
        buf[2] = (char)0; /* terminate string */
        return buf;
      }
      if ( c <= (wchar_t)0xFFFF ) { /* UTF8 == 16bit */
        buf[0] = (char)(0xd0+(c>>12 & 0x0f)); /* 1110xxxx */
